...
DEFAULT_AUDIT_MANIFEST = str(Path("config") / "audit_corpus_manifest.json")
...
def _ensure_user_site_packages() -> None:
    for vendor_name in ("_vendor_manual", "_vendor_local", "_vendor_site"):
        vendor_site = _PROJECT_ROOT / vendor_name
        ...
        for candidate in (Path(__file__), Path(sys.executable)):
            try:
                if candidate.exists() and candidate.is_file():
                    digest_source = hashlib.sha256(candidate.read_bytes()).hexdigest()
                    break
            except OSError:
                continue
        if not digest_source:
            fallback = f"{MANIFEST_VERSION}|{sys.version}|{getattr(sys, 'frozen', False)}|{__file__}"
            digest_source = hashlib.sha256(fallback.encode("utf-8", "replace")).hexdigest()
...
def _validate_input_file_limits(path: Path, limits: SecurityLimits) -> None:
    ...
    if size_bytes == 0:
        raise ValueError(f"Input file is empty and cannot be converted: {path.name}")
...
def _marker_runner_path() -> Path:
    return _PACKAGE_ROOT / "marker_sidecar_runner.py"


def _pdf_worker_runner_path() -> Path:
    return _PACKAGE_ROOT / "pdf_worker_runner.py"
...
def _extract_pdf_in_worker(
    ...
    if getattr(sys, "frozen", False):
        worker_exe = Path(sys.executable).with_name("LLMIngestWorker.exe")
        command = [str(worker_exe)] if worker_exe.exists() else [sys.executable, "--pdf-worker"]
    else:
...
def _find_marker_python(pdf_config: PDFConfig) -> Path | None:
    ...
    local_sidecar = _PROJECT_ROOT / "_python313" / "runtime" / "python.exe"
...
def _build_marker_env() -> dict[str, str]:
    ...
    for key in ("SYSTEMROOT", "WINDIR", "PATH", "PATHEXT", "COMSPEC", "PROCESSOR_ARCHITECTURE", "NUMBER_OF_PROCESSORS"):
        if os.environ.get(key):
            env[key] = os.environ[key]
    marker_tmp = _PROJECT_ROOT / "_python313" / "tmp"
    marker_tmp.mkdir(parents=True, exist_ok=True)
    marker_cache = _PROJECT_ROOT / "_python313" / "cache"
